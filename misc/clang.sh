...
ClonePrepSdClang(){
    ...
    if [ ! -e "${MainPath}/SDClang-14.1.2.0.zip" ];then
        wget -q  https://github.com/ZyCromerZ/Clang/releases/download/sdclang-14-release/SDClang-14.1.2.0.zip -O "SDClang-14.1.2.0.zip"
    fi
    unzip -P ${ZIP_PASS} SDClang-14.1.2.0.zip -d $ClangPath
    TypeBuilder="SDClang"
    ClangType="$(${ClangPath}/bin/clang --version | head -n 1)"
    [[ -z "${ZIP_PASS}" ]] && Fail="y"
    if [[ "$Fail" == "y" ]];then
